function mainn()
%MAINN Run the Phase 2 theta-gamma PAC analysis.
...
if exist('eeglab', 'file') ~= 2
    error(['EEGLAB was not found on the MATLAB path. Install EEGLAB and ', ...
           'add its directory to the path before running this function.']);
end
eeglab;
...
subj_files = { ...
    'PreProcessed/Healthy/Healthy_data_ICA_rem_trial.set', ...
    'PreProcessed/MCI/MCI_data_ICA_rem_trial.set', ...
    'PreProcessed/Mild/Mild_data_ICA_rem_trial.set'};
subj_labels = {'Healthy', 'MCI', 'Mild'};
odor_tags = [5 6];
odor_names = {'Chocolate', 'Rose'};
channels_of_interest = {'C4', 'P3', 'F8', 'Fp2', 'O1', 'Fz', 'Pz', 'Fp1', ...
    'F7', 'F3', 'F4', 'C3', 'Cz', 'T3', 'P4', 'T4', 'T5', 'O2', 'T6'};
    ...
    for o = 1:numel(odor_tags)
        ...
        for ch = 1:length(channels_of_interest)
            ...
            for tr = 1:nTr
                ...
                [theta_phase, gamma_amp] = extract_phase_amplitude( ...
                    sig, fs, method, theta_band, gamma_band);
                [mi_time_vec, t_vec, ~] = compute_MI_sliding_window( ...
                    theta_phase, gamma_amp, fs, win_len, overlap, nbins_MI);
                [mvl_vec, t_mv] = sliding_window_MVL( ...
                    theta_phase, gamma_amp, fs, win_len, overlap);
                [mi_val, P, phase_bins] = compute_MI( ...
                    theta_phase, gamma_amp, nbins_MI);
            ...
            results(s).odor(o).electrode(ch).avg_mvl_epochs = ...
                mean(mvl_trials, 'all', 'omitnan');

            if ch <= 5
                base = sprintf('plots/%s/%s_%s', ...
                    subj_labels{s}, odor_names{o}, channels_of_interest{ch});
        ...
        results(s).odor(o).avg_mvl_all_channels = mean(all_avg_mvl, 'omitnan');
        results(s).odor(o).avg_mi_all_channels = mean(all_avg_mi, 'omitnan');

        all_avg_mi_time = [];
        all_t_mi = [];
        for ch = 1:length(results(s).odor(o).electrode)
            if ~isfield(results(s).odor(o).electrode(ch), 'mi_time') || ...
                    isempty(results(s).odor(o).electrode(ch).mi_time)
                continue;
            end
            all_avg_mi_time(end + 1, :) = ...
                results(s).odor(o).electrode(ch).mi_time; %#ok<AGROW>
            all_t_mi = results(s).odor(o).electrode(ch).t_mi;
        end
        results(s).odor(o).avg_mi_time_all_channels = ...
            mean(all_avg_mi_time, 1, 'omitnan');
...
for odor_idx = 1:2
    for e = 1:length(electrodes_to_plot)
        electrode = electrodes_to_plot{e};
        plot_group_MI_comparison(results, odor_idx, electrode);
        plot_group_MVL_comparison(results, odor_idx, electrode);
    end
